...
export function mountToolStatus(container) {
 ...
 container.innerHTML='<div class="section-heading"><h2 id="status-heading">Tool status</h2><span class="subtle">Implementation progress</span></div><p class="status-explainer">Every implemented tool is listed below, followed by selected roadmap items. Available means implemented—not a live uptime guarantee. Browser and file-format limitations still apply.</p><div id="status-summary" class="status-summary"></div><div class="filters"><label class="search"><span aria-hidden="true">⌕</span><input id="status-search" type="search" placeholder="Search tool status…" aria-label="Search tool status"></label><label><span class="sr-only">Filter by status</span><select id="status-filter"><option value="all">All statuses</option><option>Available</option><option>Planned</option></select></label></div><p id="status-count" role="status"></p><div class="table-scroll status-table" role="region" aria-label="Tool implementation status" tabindex="0"><table><caption class="sr-only">Available tools and selected planned tools</caption><thead><tr><th scope="col">Tool</th><th scope="col">Category</th><th scope="col">Status</th><th scope="col">Data storage</th><th scope="col">Action</th></tr></thead><tbody id="status-rows"></tbody></table></div><p class="status-explainer">Planned entries are roadmap items, not launchable tools or delivery promises. Publishing to GitHub Pages and offline app caching remain pending.</p>';
 const $=selector=>container.querySelector(selector);
 $('#status-summary').textContent=`${tools.length} available · ${plannedTools.length} planned · ${tools.filter(t=>t.persistent).length} with local saving`;
 function render(){
